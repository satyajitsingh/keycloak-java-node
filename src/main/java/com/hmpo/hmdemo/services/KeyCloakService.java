package com.hmpo.hmdemo.services;
...
public class KeyCloakService {
	...
	public KeyCloakState createUser(UserDTO userDTO) {

		int statusId = 0;
		KeyCloakState keyCloakState = new KeyCloakState();
		try {
			...
			if (statusId == 201) {
				...
				keyCloakState.setstatusId(statusId);
				keyCloakState.setUserId(userId);
				keyCloakState.setMessage("User created in keycloak successfully");
				...
				keyCloakState.setstatusId(statusId);
				keyCloakState.setUserId("");
				keyCloakState.setMessage("User already present in keycloak");

			} else {
				System.out.println("Username==" + userDTO.getUserName() + " could not be created in keycloak");
				keyCloakState.setstatusId(statusId);
				keyCloakState.setUserId("");
				keyCloakState.setMessage("User could not be created in keycloak");
